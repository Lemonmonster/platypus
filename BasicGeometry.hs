...
data DynamicGeom = DynGeo {_dgid::(TypeRep,Int),_dgshape::PhysObj,_dgcolor::V4 Float} deriving (Typeable,Show)
...
instance EntityW '[M.Map EntityId PhysObj] '[Create DynamicGeom,Destroy] DynamicGeom [DrawableObject,PhysObj] where
  wire =
    let create a = mkSFN $ \(phyObj `HCons` HNil,_) ->
          let newObj = a & dgshape .~ phyObj
          in ((newObj,  SNil),create newObj)
    in mkObject create (const mempty) >>> signal_ _dgshape >>> drawW
