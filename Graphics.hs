  ...
  wire = mkGen $ \s (lst `ECons` size `ECons`win `ECons` ENil,  drawables `SCons` SNil) -> do
    case (size,win) of
      ([WS (V2 w h)],[window] ) -> do
        ...
        renderer <- head $ (map (return.fromDelay) lst) <|> [newRenderer 640 360]
        renderer' <- render renderer (realToFrac $ dtime s) (if null drawables then [ DO $ sprite (V2 (-200) 0) (V2 100 100) "runner" "TestAtlas"] else drawables')
        --performGC
        glSwapWindow window
