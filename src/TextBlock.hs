...
instance ZIndexed TextBox where
  zIndex = tz
...
instance Drawable TextBox where
  toDrawableObject t = DrawableObject {
    z= t^.tz,
    resources  =
    ...
    draw = \r -> do
      ...
      let mt = convertTransform ((t^.box.trans) & scale .~ pure 1) & _z._w.~ fromIntegral (t^.zIndex) :: V4 (V4 Double)
      foldM_ (\h' (x,str) -> do
          foldM_ (\x' c -> do
          ...
          return ( h' - fromIntegral (fnt^.lineSkip)) :: IO Double
