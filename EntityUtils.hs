...
mkObject :: forall s e m ein sin a sout . (HasId a, Monad m, Monoid s,
              ...
              SameLength' (MapArgs sin) sin
               ...
               =>
            (a -> Wire s e m (HList (UnmapArgs ein),HList sin) (a,SignalL sout)) ->
            (EntityId -> SignalL sout) ->
            Wire s e m (EntityL ein,SignalL (Create a ': Destroy ': sin)) ([a],SignalL sout)
mkObject create destroy =
  let mkObject' mp = mkGen $ \ ds (entIn, creates `SCons` destroys `SCons` sigIn) -> do
        let emlst = hmap ToMap entIn :: HList (MapArgs ein)
            smlst = hmap SigToMap sigIn :: HList (MapArgs sin)
            newObjects =   map ((\(C a) -> (_ident a,create a)) . _payload) creates
            (output1,updatedMap) = foldl (\(output,m) (Destroy i)->
                (output <> destroy i, M.delete (Just i) m)
              ) (mempty,mp) $ map _payload destroys
        (wireUpdate,output) <- runStateT (mapM (\(oId,oWire) -> do
              let einput = hmap (Lookup oId) emlst :: HList (UnmapArgs ein)
                  sinput = hmap (Lookup oId) smlst :: HList sin
              (out,newWire) <-lift $ stepWire oWire ds (Right (einput,sinput))
              modify (either Left (\x->fmap (\(ent,sigs) -> x <> ([ent],sigs)) out))
              return (oId,newWire)
            ) (M.toList updatedMap) ) (Right mempty)
        return (fmap (<> (map ((\(C a)-> a) . _payload) creates,output1)) output , mkObject' $ M.fromList $ wireUpdate ++ newObjects)
  in mkObject' M.empty
